...
import { ChequeService } from "../services/services/cheque.services.js";
import Busboy from "busboy";

const service = new ChequeService();

export const insertCheque = async (req: Request, res: Response) => {
  ...
  busboy.on("file", (fieldname, file, info) => {
    ...
    if(!fileName.endsWith(".csv")){
        file.resume();
        return res.status(400).json({ error: " CSV files is required" });
    }
...
export const deleteTestData = async (req: Request, res: Response) => {
  try {
    const result = await service.deleteTesData();
    res.json({
