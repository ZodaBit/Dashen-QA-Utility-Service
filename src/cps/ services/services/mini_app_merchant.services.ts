...
export class MiniAppMerchantService {
  ...
  public async insert(buffer: Buffer): Promise<string> {
    try {
      ...
      let mini_app_merchant = JSON.parse(bufferStr);
      mini_app_merchant = convertExtendedJSON(mini_app_merchant);
      mini_app_merchant = cleanEnumFields(mini_app_merchant);
      mini_app_merchant.batch_tag = test_data;
      const insertedDoc = (await MiniAppMerchantModel.insertOne(
        mini_app_merchant
      )) as any;
  ...
  public async bulkInsert(files: Express.Multer.File[]): Promise<string[]> {
    try {
      ...
      for (const file of files) {
        ...
        const bufferStr = buffer.toString("utf-8");
        let mini_app_merchant_data = JSON.parse(bufferStr);

        mini_app_merchant_data = convertExtendedJSON(mini_app_merchant_data);
        mini_app_merchant_data = cleanEnumFields(mini_app_merchant_data);
        mini_app_merchant_data.batch_tag = test_data;
        AvatarDocs.push(mini_app_merchant_data);
      }

      // Bulk insert
      const insertedDocs = await MiniAppMerchantModel.insertMany(AvatarDocs);
  ...
  delete(): Promise<any> {
    return MiniAppMerchantModel.deleteMany({ batch_tag: test_data });
  }
}
