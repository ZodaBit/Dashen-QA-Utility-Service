...
export class MiniAppCategoryService {
  ...
  public async insert(buffer: Buffer): Promise<string> {
    try {
      ...
      let mini_app_category = JSON.parse(bufferStr);
      mini_app_category = convertExtendedJSON(mini_app_category);
     mini_app_category = cleanEnumFields(mini_app_category);
      mini_app_category.batch_tag = test_data;
      const insertedDoc = (await MiniAppCategoryModel.insertOne(
        mini_app_category
      )) as any;
   ...
   public async bulkInsert(files: Express.Multer.File[]): Promise<string[]> {
      try {
        ...
        for (const file of files) {
          ...
          const bufferStr = buffer.toString("utf-8");
          let mini_app_category_data = JSON.parse(bufferStr);
  
          mini_app_category_data = convertExtendedJSON(mini_app_category_data);
          mini_app_category_data = cleanEnumFields(mini_app_category_data);
          mini_app_category_data.batch_tag = test_data;
          AvatarDocs.push(mini_app_category_data);
        }
  
        // Bulk insert
        const insertedDocs = await MiniAppCategoryModel.insertMany(AvatarDocs);
  ...
  delete(): Promise<any> {
    return MiniAppCategoryModel.deleteMany({ batch_tag: test_data });
  }
}
