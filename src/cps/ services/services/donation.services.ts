...
export class DonationService {
  ...
  public async insert(buffer: Buffer): Promise<string> {
    try {
      ...
      let donation = JSON.parse(bufferStr);
      donation = convertExtendedJSON(donation);
     donation = cleanEnumFields(donation);
      donation.batch_tag = test_data;
      const insertedDoc = (await DonationModel.insertOne(
        donation
      )) as any;
   ...
   public async bulkInsert(files: Express.Multer.File[]): Promise<string[]> {
      try {
        ...
        for (const file of files) {
          ...
          const bufferStr = buffer.toString("utf-8");
          let donation_data = JSON.parse(bufferStr);
  
          donation_data = convertExtendedJSON(donation_data);
          donation_data = cleanEnumFields(donation_data);
          donation_data.batch_tag = test_data;
          AvatarDocs.push(donation_data);
        }
  
        // Bulk insert
        const insertedDocs = await DonationModel.insertMany(AvatarDocs);
  ...
  delete(): Promise<any> {
    return DonationModel.deleteMany({ batch_tag: test_data });
  }
}
