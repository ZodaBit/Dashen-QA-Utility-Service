...
export class AvatarService {
  ...
  public async insert(buffer: Buffer): Promise<string> {
    try {
      ...
      let avatar = JSON.parse(bufferStr);
      avatar = convertExtendedJSON(avatar);
      avatar = cleanEnumFields(avatar);
      avatar.batch_tag = test_data;
      const insertedDoc = (await AvatarModel.insertOne(
        avatar
      )) as any;
   ...
   public async bulkInsert(files: Express.Multer.File[]): Promise<string[]> {
      try {
        ...
        for (const file of files) {
          ...
          const bufferStr = buffer.toString("utf-8");
          let avatar_data = JSON.parse(bufferStr);
  
          avatar_data = convertExtendedJSON(avatar_data);
          avatar_data = cleanEnumFields(avatar_data);
          avatar_data.batch_tag = test_data;
          AvatarDocs.push(avatar_data);
        }
  
        // Bulk insert
        const insertedDocs = await AvatarModel.insertMany(AvatarDocs);
  ...
  delete(): Promise<any> {
    return AvatarModel.deleteMany({ batch_tag: test_data });
  }
}
