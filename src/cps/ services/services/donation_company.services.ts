...
export class DonationCompanyService {
  ...
  public async insert(buffer: Buffer): Promise<string> {
    try {
      ...
      let donation_company = JSON.parse(bufferStr);
      donation_company = convertExtendedJSON(donation_company);
     donation_company = cleanEnumFields(donation_company);
      donation_company.batch_tag = test_data;
      const insertedDoc = (await DonationCompanyModel.insertOne(
        donation_company
      )) as any;
   ...
   public async bulkInsert(files: Express.Multer.File[]): Promise<string[]> {
      try {
        ...
        for (const file of files) {
          ...
          const bufferStr = buffer.toString("utf-8");
          let donation_company_data = JSON.parse(bufferStr);
  
          donation_company_data = convertExtendedJSON(donation_company_data);
          donation_company_data = cleanEnumFields(donation_company_data);
          donation_company_data.batch_tag = test_data;
          AvatarDocs.push(donation_company_data);
        }
  
        // Bulk insert
        const insertedDocs = await DonationCompanyModel.insertMany(AvatarDocs);
  ...
  delete(): Promise<any> {
    return DonationCompanyModel.deleteMany({ batch_tag: test_data });
  }
}
