...
export class NotificationService {
  ...
  public async insert(buffer: Buffer): Promise<string> {
    try {
      ...
      let notification = JSON.parse(bufferStr);
      notification = convertExtendedJSON(notification);
     notification = cleanEnumFields(notification);
      notification.batch_tag = test_data;
      const insertedDoc = (await NotificationModel.insertOne(
        notification
      )) as any;
   ...
   public async bulkInsert(files: Express.Multer.File[]): Promise<string[]> {
      try {
        ...
        for (const file of files) {
          ...
          const bufferStr = buffer.toString("utf-8");
          let notification_data = JSON.parse(bufferStr);
  
          notification_data = convertExtendedJSON(notification_data);
          notification_data = cleanEnumFields(notification_data);
          notification_data.batch_tag = test_data;
          AvatarDocs.push(notification_data);
        }
  
        // Bulk insert
        const insertedDocs = await NotificationModel.insertMany(AvatarDocs);
  ...
  delete(): Promise<any> {
    return NotificationModel.deleteMany({ batch_tag: test_data });
  }
}
