...
export class DonationCategoryService {
  ...
  public async insert(buffer: Buffer): Promise<string> {
    try {
      ...
      let donation_Category = JSON.parse(bufferStr);
      donation_Category = convertExtendedJSON(donation_Category);
     donation_Category = cleanEnumFields(donation_Category);
      donation_Category.batch_tag = test_data;
      const insertedDoc = (await DonationCategoryModel.insertOne(
        donation_Category
      )) as any;
   ...
   public async bulkInsert(files: Express.Multer.File[]): Promise<string[]> {
      try {
        ...
        for (const file of files) {
          ...
          const bufferStr = buffer.toString("utf-8");
          let donation_categories = JSON.parse(bufferStr);
  
          donation_categories = convertExtendedJSON(donation_categories);
          donation_categories = cleanEnumFields(donation_categories);
          donation_categories.batch_tag = test_data;
          AvatarDocs.push(donation_categories);
        }
  
        // Bulk insert
        const insertedDocs = await DonationCategoryModel.insertMany(AvatarDocs);
  ...
  delete(): Promise<any> {
    return DonationCategoryModel.deleteMany({ batch_tag: test_data });
  }
}
