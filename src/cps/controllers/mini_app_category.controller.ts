...
const upload = multer({
  dest: "uploads/mini_app_category/",
...
export const insert = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        message: "Inserted mini app category successfully.",
        insertedIds: ids,
      });
    } catch (err: any) {
      console.error(err);
...
export const bulkInsert = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        message: `Inserted ${insertedIds.length} mini app category in bulk successfully`,
        insertedIds,
      });
    } catch (err: any) {
      return res.status(500).json({ error: err.message });
