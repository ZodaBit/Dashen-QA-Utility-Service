...
const upload = multer({
  dest: "uploads/donation_category/",
...
export const insert = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        message: "Inserted donation category successfully.",
        insertedIds: ids,
      });
    } catch (err: any) {
      console.error(err);
...
export const bulkInsert = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        message: `Inserted ${insertedIds.length} donation category in bulk successfully`,
        insertedIds,
      });
    } catch (err: any) {
      return res.status(500).json({ error: err.message });
